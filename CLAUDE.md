...
- **EO-only generator.** `gen_dtxqcd_cfgs` uses `DTXQCDWilsonCloverRationalEOAction` + `DTXQCDLogDetCloverEOAction`. The `tests/dtxqcd/Test_dtxqcd_2pt_gencfgs` driver supports `USE_FULL_PF=1` (non-EO) for ablation. `AUX_INIT_AUTO=1` bisects to the self-consistent saddle (needed at small λ).
