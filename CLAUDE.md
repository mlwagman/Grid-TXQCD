...
- **Production generator capabilities** (all in `gen_dtxqcd_cfgs.cc`): checkpoint **resume** (scans `ckpoint_lat{,_daux,_rng}.<t>`, NERSC-checksum-verified), **runtime integrator selection** (`INTEGRATOR=MinimumNorm2|ForceGradient`; both templates compiled — default `MinimumNorm2`), the **signed-Pfaffian diagnostics observer** `production/dtxqcd_diag.h` (per-traj γ5·M₄₈ low eigenvalues → `signPf`, aux VEVs/Tr M⁻¹/correlators/per-action Fdt → `hmc_diagnostics.<traj>.h5`, consumed by `analyze_sign_reweighting.py`), and **`IMPORT_CFG=<path>`** to fork a thermalized chroma gauge config (sniffs magic: `BEGIN_HEADER`→NERSC, else ILDG/`.lime` via `IldgReader` under `#ifdef HAVE_LIME` — `build-gpu` is reconfigured `--with-lime`; verified: chroma `cl3_16_48_b6p1` reads back plaq 0.5133 = the QCD value). The resume scan runs first, so `IMPORT_CFG` only fires into a fresh (empty) `cfg_dir` — give a unique `DTXQCD_SUFFIX`.
- **Production launch scripts** (committed): `production/run_dtxqcd_gencfgs.sh` is the canonical reusable launcher with every production knob ON by default (GPU/cuBLAS paths, `AUX_INIT_AUTO`, `RAT_AUTO_HI`, stout, MN2 MDS=10 trajL=√2/4) at the chroma-matched 16³×48 point — all `${VAR:-default}` overridable; uses `srun_gpu_wrapper.sh` (one GPU/rank, `--shm-mpi 1`). `slurm_dtxqcd_stab_{chromafork,weakfield}.sh` are the integrator-stability-test sbatch wrappers. **Trajectory-count convention (`gen_dtxqcd_cfgs.cc:472`): executed = `TRAJ − NO_METROP − start_traj`**, so for N executed trajectories set `TRAJ = N + NO_METROP` (`TRAJ == NO_METROP` runs **zero**).
...
**Production settings.** Canonical for 16³×48 (matches chroma gauge-gen 1:1): `INTEGRATOR=MinimumNorm2 MDSTEPS=10 TRAJL=√2/4` — the ¼ rescale gives traj-to-traj correspondence with chroma `n_steps=10`. The older `INTEGRATOR=ForceGradient MDSTEPS=20-28 TRAJL=√2` (~50-70% acceptance) is equivalent at the same physical step size. Multi-rate (TXQCD production): gauge sub-integrator ×4 finer than fermion, aux ×1 (slowest). **DTXQCD's `run_dtxqcd_gencfgs.sh` launcher sets `GAUGE_MULT=8`, `AUX_MULT=1`** (`ActionSet=[L1 fermion ×1 coarsest, L2 gauge ×8, L3 aux ×1 innermost]`). Gauge ×8 matches TXQCD's effective gauge substeps (4×INNER 2): the earlier ×2 default **overshot** the large weak-field plaq≈1 start force (DTXQCD weak-field λ=6 swung 0.53→0.44 vs TXQCD's smooth monotone 0.69→0.53→…); ×8 tracks the descent. Because aux is nested *under* gauge, `aux ×1` with `gauge ×8` keeps the aux step size identical to the old `gauge ×2 / aux ×4` (`trajL/(10·8·1)=trajL/(10·2·4)=trajL/80`), so aux `Fdt`≈0.05 and aux overhead are unchanged — only the gauge is 4× finer (gauge force is cheap, ~24 s/traj at ×2). The equilibrium is independent of substep count (Metropolis); this only smooths the thermalization path + dH. (`gen_dtxqcd_cfgs.cc` bare-run fallback is still 2/4, overridden by the launcher.) Override via `GAUGE_MULT`/`AUX_MULT`. See `production/HMC_CONVENTIONS.md` and `production/HMC_TUNING_LESSONS.md`. `NO_METROP=K` force-accepts the **first K trajectories** (not just traj 0, and not the whole run). Watch `Fdt max` in the log: <0.5 comfortable, >1 unstable, >10 catastrophic.
