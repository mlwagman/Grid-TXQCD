...
- **Fresh-start aux init.** `AUX_INIT_AUTO=1` finds the self-consistent saddle by **Picard-first** iteration (`Σ_{n+1}=Σ_DTXQCD(Σ_n)` from aux=0; Jacobian `~N_f/λ²` so it contracts in ~2 evals at large λ — at λ=10 the saddle ≈ mean-field to <0.1%) with a **bisection fallback** when not contracting (small λ). Tol is `AUX_INIT_TOL` (default **0.1** — 10% is ample for an HMC init; the trajectory equilibrates the rest). Other knobs: `AUX_INIT=<Σ>` (explicit, skip the solve), `AUX_FLUCT_LAMBDA` (aux draw width, default 10), `AUX_INIT_PICARD[_MAX]`, `AUX_INIT_MAX_ITER`, `ZERO_DN_INIT`/`ZERO_ALL_AUX`, `VEV_NOISE`, `G5M_NEV`/`G5M_NKRYLOV`/`G5M_EVALS_OFF`. Tier-2 TODO: `DTXQCDSmearedConfiguration.h` (stout) does not exist yet — the generator runs **csw=0 only** until it's written; the chroma-matched csw=1.249 point needs it.
