...
**Production settings.** Canonical for 16³×48 (matches chroma gauge-gen 1:1): `INTEGRATOR=MinimumNorm2 MDSTEPS=10 TRAJL=√2/4` — the ¼ rescale gives traj-to-traj correspondence with chroma `n_steps=10`. The older `INTEGRATOR=ForceGradient MDSTEPS=20-28 TRAJL=√2` (~50-70% acceptance) is equivalent at the same physical step size. Multi-rate (TXQCD production): gauge sub-integrator ×4 finer than fermion, aux ×1 (slowest). **DTXQCD's `gen_dtxqcd_cfgs` defaults differ** — `ActionSet=[L1 fermion ×1 (coarsest, fewest steps), L2 gauge ×`GAUGE_MULT`=2, L3 aux ×`AUX_MULT`=4 (finest)]`; the expensive fermion force is evaluated least often, cheap aux most often, and aux ×4 keeps aux `Fdt` ≈0.14 (×1 would push it ~0.56, marginal). Override via `GAUGE_MULT`/`AUX_MULT`. See `production/HMC_CONVENTIONS.md` and `production/HMC_TUNING_LESSONS.md`. `NO_METROP=K` force-accepts the **first K trajectories** (not just traj 0, and not the whole run). Watch `Fdt max` in the log: <0.5 comfortable, >1 unstable, >10 catastrophic.
