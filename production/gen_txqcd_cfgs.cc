...
int main(int argc, char **argv) {
  ...
  if (latest > 0) {
    ...
    if (const char *si = std::getenv("AUX_INIT"); si && *si) {
      ...
      // Σ_{n+1} = Σ_TXQCD(Σ_n).  Seed Σ_0 is the bare (csw=0, periodic) Wilson
    ...
    if (start_type == "hot") {
      ...
      // Each evaluation refills the aux fields at a trial Σ, builds the FULL
      ...
      // Σ_{n+1} = Σ_TXQCD(Σ_n) from Σ_0 = 0 (bare mean field), accepting when the
      // relative change falls below AUX_INIT_TOL; the TXQCD map contracts at
      ...
      if (auto_measure) {
        ...
        RealD aux_iter_tol = 0.1;  // 10% is plenty for an HMC init — the
                                   // trajectory equilibrates the residual.
        if (const char *t = std::getenv("AUX_INIT_TOL"); t && *t)
          aux_iter_tol = std::atof(t);
        int picard_max =
            TXQCDProduction::detail::env_int("AUX_INIT_PICARD_MAX", 6);
