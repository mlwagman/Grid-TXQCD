...
LOG=logs/smoke_b6p3_lam6_48cube_hasen.log
mkdir -p logs
...
# HMC env
export LAMBDA=6
export SUFFIX=_b6p3_smoke_lam6_hasen
export N_TRAJ=1
export NO_METROP=1
...
# 4-node 16-rank, mpi=1.1.4.4 → per-rank 48 x 48 x 12 x 24.
# Cuts per-rank lattice 2× vs the 2-node split, giving ~10 GB peak per
# rank for HMC at full speed (PRECOMPUTE_GPU=1 + MG + multishift Phi).
