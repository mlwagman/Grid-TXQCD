...
#SBATCH --output=slurm-logs/disco_hiL.%j.out
...
run_one () {
  ...
  local logfile="logs/disco_sparseHI_${kind}_${lam}${suffix}_t${traj}_gpu${gpu}.log"
...
while true; do
  ...
  echo ""
  echo "###########################################################"
  echo "#  DISCO HIGH-λ PASS $PASS  $(date +'%H:%M:%S')"
  echo "###########################################################"
  ANYTHING_RUN=0
