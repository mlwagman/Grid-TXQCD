// Style B Phase 2.5 Session 3d — out-of-line strong definition for the Gamma5
// native kernel. Compiled in a clean TU (no `using namespace Grid;`) so trove's
// global `detail` namespace doesn't collide with `Grid::detail`.
//
// DO NOT include dtxqcd_quda_gamma5_native.h here — that header defines the
// same function as `inline`, which would cause an in-TU redefinition.
// Instead, this file provides a standalone strong definition with identical
// body, which the linker picks over the inline weak symbols emitted by tests
// that include _native.h directly.
...
void ApplyGamma5InplaceNative(quda::ColorSpinorField &io_csf, int variant) {
