// Step (Symanzik): Auxiliary field correlators from stout-smeared TXQCD configs.
...
int main(int argc, char **argv) {
  ...
  TXQCDField U(&Grid);
  for (int traj : trajs) {
    std::cout << GridLogMessage << "[aux symanzik optlam] traj=" << traj << std::endl;
  ...
  std::cout << GridLogMessage << "Auxiliary correlators written to "
            << meas_dir() << "/meas_txqcd_aux.h5" << std::endl;
  Grid_finalize();
  return 0;
}
