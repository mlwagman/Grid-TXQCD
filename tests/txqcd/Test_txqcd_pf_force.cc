...
static RealD TensorTrInner(const LatticeTField &E, const LatticeTField &F) {
  // `total += ...` inside a thread_for is a data race across threads; the
  // original pattern here gave a non-deterministic AN on GPU builds
  // (different AN every run despite a deterministic force).  Lattices are
  // tiny in this test; a plain serial loop is fine.
