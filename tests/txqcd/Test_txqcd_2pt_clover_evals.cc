...
// for WilsonClover operator (csw=1, no smearing) on clover test configs.
...
int main(int argc, char **argv) {
...

  {
    TXQCDField U(&Grid);
    for (int traj : trajs) {
      std::cout << GridLogMessage << "[evals] TXQCD clover traj=" << traj << std::endl;
      LoadTxqcdConfig(U, sRNG, pRNG, traj);
      pRNG.SeedFixedIntegers({100, 200, 300, 400, 500});
      WCF Dw(U.U, Grid, RBGrid, mass, csw, csw);
  ...
  }
  {
    ...
    for (int traj : trajs) {
      std::cout << GridLogMessage << "[evals] QCD clover traj=" << traj << std::endl;
