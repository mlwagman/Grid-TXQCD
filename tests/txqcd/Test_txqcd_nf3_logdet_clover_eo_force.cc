// Nf=3 force-consistency test for TXQCDLogDetCloverEOAction with
...
// 3. Gauge force is zero (Mee doesn't depend on gauge links) at csw=0
...
// At Nf=3 both LatticeSigmaField (Nf×Nf) and LatticeSFieldC (Nc×Nc) are the
// same Lattice<iScalar<iScalar<iMatrix<vComplex,3>>>> type, so a single
// templated helper covers both.
...
int main(int argc, char **argv) {
  ...
  TXQCDLogDetCloverEOAction action(Grid, RBGrid, mass);
  ...
  {
    RealD csw = 1.0;
    TXQCDLogDetCloverEOAction caction(Grid, RBGrid, mass, csw);
