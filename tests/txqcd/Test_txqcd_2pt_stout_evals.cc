...
// for WilsonClover operator (csw=1) on stout-smeared test configs.
...
int main(int argc, char **argv) {
...

  {
    TXQCDField U(&Grid);
    for (int traj : trajs) {
      std::cout << GridLogMessage << "[evals] TXQCD stout traj=" << traj << std::endl;
  ...
  }
  {
    ...
    for (int traj : trajs) {
      std::cout << GridLogMessage << "[evals] QCD stout traj=" << traj << std::endl;
