// Step 3 (Stout): Disconnected pion loops and VEVs using Wilson-Clover.
...
int main(int argc, char **argv) {
  ...
  // TXQCD
  {
    TXQCDField U(&Grid);
    for (int traj : trajs) {
      std::cout << GridLogMessage << "[disc optlam] TXQCD stout traj=" << traj << std::endl;
  ...
  // QCD
  {
    ...
    for (int traj : trajs) {
      std::cout << GridLogMessage << "[disc optlam] QCD stout traj=" << traj << std::endl;
  ...
  std::cout << GridLogMessage << "Disconnected + VEV stout measurements written to "
