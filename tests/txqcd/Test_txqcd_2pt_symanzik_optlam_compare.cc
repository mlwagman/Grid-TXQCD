// Step (Symanzik): Compare TXQCD and QCD 2pt measurements (Fierz test).
// Same analysis as Wilson version but reads from symanzik measurement directory.
...
int main(int argc, char **argv) {
  ...
  std::cout << GridLogMessage << "TXQCD symanzik samples: " << N
            << "   QCD symanzik samples: " << M << std::endl;
  ...
  std::cout << GridLogMessage
            << (exitcode ? "SOME SYMANZIK 2pt CHECKS FAILED"
                         : "ALL SYMANZIK 2pt CHECKS PASSED")
            << std::endl;
  Grid_finalize();
  return exitcode;
}
