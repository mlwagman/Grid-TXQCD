...
// for Wilson fermion operator (csw=0, no smearing) on base test configs.
...
SmallestEvalsMdagM(WilsonFermionD &Dw, GridCartesian &Grid, GridParallelRNG &pRNG, int Nev) {
  MdagMLinearOperator<WilsonFermionD, LatticeFermion> MdagM(Dw);
...
int main(int argc, char **argv) {
  ...
  std::vector<std::vector<RealD>> evals_tx, evals_qcd;

  {
    ...
    for (int traj : trajs) {
      ...
      WilsonFermionD Dw(U.U, Grid, RBGrid, mass);
      auto ev = SmallestEvalsMdagM(Dw, Grid, pRNG, Nev);
      evals_tx.push_back(ev);
      std::cout << GridLogMessage << "  |lambda(g5M)|:";
      for (auto e : ev) std::cout << " " << e;
      std::cout << std::endl;
    }
  }
  {
...

  {
    ...
    write(wr, "evals_light", evals_tx);
  }
  {
    Hdf5Writer wr(meas_dir() + "/meas_qcd_evals.h5");
    write(wr, "evals_light", evals_qcd);
  }

  RealD min_eval = 1e10;
  for (auto &ev : evals_tx) for (auto e : ev) min_eval = std::min(min_eval, e);
  for (auto &ev : evals_qcd) for (auto e : ev) min_eval = std::min(min_eval, e);
