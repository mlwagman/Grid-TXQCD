// Test_dtxqcd_mooee_n: agreement between single-RHS Mooee (per-site SIMD via
// DtxqcdApplyMooeeDoubled) and the multi-RHS MooeeN (DTXQCD_MOOEE_CUBLAS path
// or per-RHS Mooee fallback) on the cached doubled-Wilson-Clover EO operator.
//
// Generates NRHS = 5 random doubled fermions on each CB, applies Mooee once-per-
// RHS via the single-RHS SIMD path AND once via the batched MooeeN path, and
// requires the outputs to agree to machine precision.  The batched path either
// loops single-RHS Mooee (default, bit-identical) or routes through the cuBLAS
// gemmBatched(48, NRHS, 48) reading the precomputed forward 48x48 device matrix
// fwd_dev (DTXQCD_MOOEE_CUBLAS=1 path).
//
// Run:
//   ./tests/dtxqcd/Test_dtxqcd_mooee_n --grid 4.4.4.4 --mpi 1.1.1.1   # default
//   DTXQCD_MOOEE_CUBLAS=1 ./tests/dtxqcd/Test_dtxqcd_mooee_n --grid 4.4.4.4 \
//                          --mpi 1.1.1.1                                # cuBLAS
...
int main(int argc, char **argv) {
  ...
  pRNG.SeedFixedIntegers({711, 712, 713, 714});
  ...
  auto run_csw = [&](RealD csw, const char *tag) {
    ...
    for (int cb_idx = 0; cb_idx < 2; ++cb_idx) {
      ...
      // Single-RHS: per-RHS Mooee (SIMD path).
      for (int k = 0; k < NRHS; ++k) Dw.Mooee(ins[k], outs_single[k]);

      // Batched: one MooeeN call.
      ...
      Dw.MooeeN(in_ptrs, out_ptrs);

      RealD maxd = 0.0;
      for (int k = 0; k < NRHS; ++k)
        maxd = std::max(maxd, MaxAbsDiff(outs_single[k], outs_batched[k]));
      check((std::string(tag) + " " + cbname + " Mooee  vs MooeeN").c_str(),
            maxd, 1e-10);
